...
class ProfilesController extends Controller {
    ...
    public function actionCreate() {
        ...
        if (isset($_POST['Profiles'])) {
            ...
            //         var_dump($_POST);
            if ($model->profile_name == "") {
                Yii::app()->user->setFlash('error', 'You must set a profile name');
                $this->redirect(array('admin'));
            }
            $model->cdate = date('Y-m-d');
            $model->lmdate = date('Y-m-d');
            $users = Users::model()->findAll('username=:x', array(':x' => Yii::app()->user->name));
            foreach ($users as $user)
                ;
            if (count($users) == 0)
                $model->muser = 0;
            else
                $model->muser = $user->user_id;
            if ($model->save())
                $this->redirect(array('admin'));
            exit;
        }
    ...
    public function actionAdmin() {
        ...
        $profiles = Profiles::model()->findAll();
