...
<?php echo $rate = RateTypes::model()->findByPk((InstitutionsQuotation::model()->findByPk(Bqpl::model()->findByPk($id)->num)->quotation_id))->rt_name ?>
</h3><hr>
    <?php $iblentry = Bqpl::model()->findByPk($id); ?>
<p>Parties : <?php echo $cname = Bqcus::model()->findByPk($iblentry->ban)->resnam; ?> and <?php echo Bqcus::model()->findByPk($iblentry->cus)->resnam; ?></p>
<?php $detailibls = Bqdpl::model()->findAll('fd_id=:x', array(':x' => $id)); ?>
<?php foreach ($detailibls as $detailibl) { ?>
    <div class='panel-body'>
        ...
        ;
                ?> for <?php echo $rate; ?>. The quotation was done for <?php echo Bqpl::model()->findByPk($id)->amo . " at rate  " . Bqpl::model()->findByPk($id)->prat . " %"; ?> .</p>
                            </div>
                        </div>    
                    </td>
                </tr>
