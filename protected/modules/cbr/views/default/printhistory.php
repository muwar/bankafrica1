...
<?php echo $rate = RateTypes::model()->findByPk((InstitutionsQuotation::model()->findByPk(Bqcb::model()->findByPk($id)->num)->quotation_id))->rt_name ?>
</h3><hr>
    <?php $iblentry = Bqcb::model()->findByPk($id); ?>
<p>Parties : <?php echo $cname = Bqcus::model()->findByPk($iblentry->lban)->resnam; ?> and <?php echo Bqcus::model()->findByPk($iblentry->cus)->resnam; ?></p>
<?php $detailibls = Bqdcb::model()->findAll('cb_id=:x', array(':x' => $id)); ?>
<?php foreach ($detailibls as $detailibl) { ?>
    <div class='panel-body'>
        ...
        ;
                ?> for <?php echo $rate; ?>. The quotation was done for <?php echo Bqcb::model()->findByPk($id)->amo . " at rate  " . Bqcb::model()->findByPk($id)->lrat . " %"; ?> .</p>
                            </div>
                        </div>    
                    </td>
                </tr>
