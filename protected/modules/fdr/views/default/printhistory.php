...
<?php echo $rate = RateTypes::model()->findByPk((InstitutionsQuotation::model()->findByPk(Bqfd::model()->findByPk($id)->num)->quotation_id))->rt_name ?>
</h3><hr>
    <?php $iblentry = Bqfd::model()->findByPk($id); ?>
<p>Parties : <?php echo $cname = Bqcus::model()->findByPk($iblentry->rban)->resnam; ?> and <?php echo Bqcus::model()->findByPk($iblentry->cus)->resnam; ?></p>
<?php $detailibls = Bqdfd::model()->findAll('fd_id=:x', array(':x' => $id)); ?>
<?php foreach ($detailibls as $detailibl) { ?>
    <div class='panel-body'>
        ...
        ;
                ?> for <?php echo $rate; ?>. The quotation was done for <?php echo Bqfd::model()->findByPk($id)->amo . " at rate  " . Bqfd::model()->findByPk($id)->drat . " %"; ?> .</p>
                            </div>
                        </div>    
                    </td>
                </tr>
