...
// using the default layout 'protected/views/layouts/main.php'
   ...
   public function actionChangepwd() {
        try {
            ...
            $checkmodel = current(Evuti::model()->findAll('nom=:x', array(':x' => Yii::app()->user->name)));
            
            $model = Evuti::model()->findByPk($checkmodel->id);

            if ($model->pswd != md5($_GET['opwd'])) {
                echo 'incorrect';
            } else {
                $model->pswd = md5($_GET['npwd']);
              //  $model->update();
                if ($model->update()) {
                    echo 'true';
                } else {
   ...
   public function actionLangchange() {
        try {
             ...
             Yii::app()->session['language_chosen'] = $_GET['lang'];
             echo 'true';
    ...
    public function actionCountrychange() {
        try {
             ...
             Yii::app()->session['country_chosen'] = $_GET['country'];
             echo 'true';
